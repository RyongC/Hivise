...
def btn_exe():
    ...
    arr_raw = np.full(shape=raw_shape, fill_value=0, dtype='float32')
    ...
    # np.savetxt(f'./{txt_path.get()}.csv', final, delimiter = '\t', fmt = '%0.2f')
    return
...
def merged(sorteds):  # path / 정렬된 텍스트 이터레이터 활용 -> {매개변수 폴더이름 : 머지 배열(ndim : 1)}
    with open(f'{txt_path.get()}/{sorteds}', 'r') as txt:
        ...
        arr = np.array(tmp3, dtype='float32').reshape(40, 40)
    return arr
...
def txt_lst(folders):
    ...
    txt_path.set(path)
    lst = glob('*h*m*s.txt', root_dir=txt_path.get())  # <- 비정렬 텍스트 리스트
    for i in lst:
        idx_txt(i)
